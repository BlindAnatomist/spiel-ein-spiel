...
Executed September 25, 2026 UTC. All scheduled categories completed.
...
The executable plan is reports/bot-validation/manifest.json. Detailed per-game evidence is in reports/bot-validation/detailed/*.json.gz. summary.json contains estimates, cluster counts, ordinary and adjusted intervals. COMPARISONS.txt and DEALER_AND_LONERS.txt are readable appendices. fixtures.json.gz and tactical-summary.json retain targeted evidence. provenance.json identifies the source commit, exact harness source hashes, commands and validation logs. EXPERIMENT.md explains the design and field mappings.

Resume only unfinished simulation chunks with:
node src/audit/validation-cli.ts --mode run --workers 4

There are no unfinished chunks in this package. Recompute statistics without rerunning games with:
python3 scripts/analyze-bot-validation.py
python3 scripts/report-bot-validation.py

Analysis requires Python 3 and NumPy; the recorded environment used Python 3.12 and NumPy 2.3.5. Simulation uses Node 24.19.0 and the repository's locked dependencies. The result ZIP includes the report, complete manifest, detailed results, readable appendices, harness sources and the publication commit receipt. No production configuration change or automatic strategy update is part of this audit.
