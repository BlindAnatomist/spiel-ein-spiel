...
The fixed post-narration quiet guard is **1150 ms**. The existing estimated speech budget is unchanged. At the next automatic focus boundary, the controller waits for queued messages/reviews to finish and clear, then waits 1150 ms before rendering the interactive state and moving focus. A review arriving during that guard is allowed to finish and clear, followed by a fresh guard. Cancellation prevents old focus. No guard is added to intervening bot decisions (their existing 350 ms pacing is retained), or to focus with no new narration. The focus algorithm, result focus and hand order are unchanged. Sound cues never join the wait chain.
