...
test('automatic event clears before the 1150 ms guard starts; first legal card focuses only after guard', async () => {
  ...
  assert.deepEqual(f.log.slice(-2),['clear','wait:1150']);
  ...
  assert.equal(FOCUS_GUARD_MS,1150);
});
test('West order-up and pickup both finish before guarded first-discard focus', async () => {
  ...
  f.waits.shift()!.release();await flush();
  assert.equal(f.waits[0]!.ms,1600);assert.equal(f.log.includes('wait:1150'),false);
  f.waits.shift()!.release();await flush();assert.equal(f.log.at(-1),'wait:1150');
  f.waits.shift()!.release();await pending;
...
test('quiet guard does not delay un-narrated focus or each intervening bot decision', async () => {
  ...
  assert.equal(g.log.includes('wait:1150'),false);
  g.waits.shift()!.release();await flush();assert.equal(g.waits[0]!.ms,1150);
  g.waits.shift()!.release();await pending;
  assert.equal(g.log.filter(x=>x==='wait:1150').length,1);
});
test('review requested during quiet guard finishes and receives a fresh quiet interval', async () => {
  ...
  speech.release();await review;await flush();assert.equal(f.log.at(-1),'wait:1150');
...
test('stopping during the guard prevents stale focus; hand/game results retain guarded result focus', async () => {
  ...
  for(const phase of ['hand-over','game-over'] as const) {
    ...
    g.waits.shift()!.release();await flush();assert.equal(g.log.at(-1),'wait:1150');
