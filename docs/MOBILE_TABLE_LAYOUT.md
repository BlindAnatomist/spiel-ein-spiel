...
Engine, policies, announcer, presentation summaries, session, main entry/randomness and sound are unchanged. The controller mirrors PR #6's real-device remediation by extending only the narrator-to-focus quiet guard from 750 ms to 1150 ms; focus selection itself is unchanged. First-legal focus selection and focus bookkeeping in render.ts remain unchanged. Existing coverage is preserved, with an added swipe-order regression for hand → bids → Pass → state → last trick. Full matches, hidden-information and determinism tests remain required to stay green.
...
## Owner testing notes — next PR #7 refinement pass (2026-09-24)

These are accepted design notes from owner/Cynthia testing. They are recorded here for the next focused PR #7 refinement pass and are not yet implemented.
...
Replace the current decorative star/called-suit icon on Repeat current state with one fixed target/bullseye-style icon. The icon no longer changes when a suit is called. Suit glyphs should mean suits; the state control should have its own stable visual identity.

Keep the control's accessible identity as Repeat current state, but append only the most frequently needed fact to its accessible name:
...
Reorder the activated Current state report by decision priority rather than bookkeeping priority. During play, report:
1. called suit/trump;
...
When a sighted player activates Repeat current state, render the same summary text visibly in a compact panel. Do not create parallel summary logic: the visible panel and VoiceOver announcement must derive from the same public summary/source of truth.
...
The initial game-start control should display explicit text, preferably "Start game", rather than relying on an icon. The small space cost is justified by immediate recognizability.

Add a compact Help/Instructions control in the top area without crowding the table. Its explanation should be short and practical, including at minimum:
