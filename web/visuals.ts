...
import { names } from './presentation.ts';
...
export function renderVisualTable(root: HTMLElement, v: PlayerView) {
  ...
  caller.textContent=v.caller===null ? `Round ${v.biddingRound}` : `${names[v.caller]}${v.alone?' · alone':''}`;
  ...
  const icon=root.querySelector<HTMLElement>('#repeat-state .state-icon')!;
  icon.dataset.glyph=v.trump ? suitGlyph[v.trump] : '★';
