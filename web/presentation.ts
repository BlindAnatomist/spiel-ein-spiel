...
export function events(before: PlayerView, after: PlayerView, actor: Seat, action: Action, seatNames: SeatNames = names): string[] {
  ...
  if (actor !== 0) {
    if (action.type === 'play') messages.push(`${seatNames[actor]} ${before.trick.length === 0 && (after.trick.some(p => p.seat === actor && p.card === action.card) || after.completedTricks.length > before.completedTricks.length) ? 'leads' : 'plays'} ${cardName(action.card, after.trump).toLowerCase()}.`);
    else if (action.type === 'discard') messages.push(`${seatNames[actor]} discards a card.`);
    else if (action.type === 'pass' || action.type === 'order-up' || action.type === 'call') messages.push(`${seatNames[actor]} ${action.type === 'pass' ? 'passes' : action.type === 'order-up' ? `orders up ${after.trump}${action.alone ? ' and goes alone' : ''}` : `calls ${action.suit}${action.alone ? ' and goes alone' : ''}`}.`);
  }
  if (before.upCardStatus !== after.upCardStatus) {
    if (after.upCardStatus === 'turned-down') messages.push('The up-card is turned down.');
    if (after.upCardStatus === 'ordered') messages.push(`${seatNames[after.dealer]} ${after.dealer === 0 ? 'pick' : 'picks'} up.`);
  }
  if (after.completedTricks.length > before.completedTricks.length) {
    const winner = after.completedTricks.at(-1)!.winner;
    messages.push(`${seatNames[winner]} ${winner === 0 ? 'take' : 'takes'} the trick.`);
  }
